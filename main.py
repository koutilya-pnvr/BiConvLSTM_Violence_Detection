...
model_names = ['E', 'E_bi', 'E_bi_avg_pool', 'E_bi_max_pool', 'max_pool']
data_names = ['H', 'M', 'VF']
...
parser.add_argument('data_name', default='H',
                    ...
                    ' | '.join(data_names) +
                    ' (default: H)')
parser.add_argument('--arch', '-a', metavar='ARCH', default='E',
                    choices=model_names,
                    help='model architecture: ' +
                    ' | '.join(model_names) +
                    ' (default: E)')
parser.add_argument('--t', '--transform', dest='transform', default='R', type=str,
...
def run_training(train_loader, val_loader, model, criterion, optimizer, best_prec):
    ...
    while True:
        ...
        save_checkpoint({
            ...
            }, is_best, args.arch + "_fold_" + str(args.kfold))
        epoch += 1
...
def network_factory(arch):
    ...
    if arch == 'E':
        ...
        from networks.max_pool import VP
